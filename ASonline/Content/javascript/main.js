...
;(function($) {
   ...
   'use strict'
    ...
    var hashedCookiesUserId = function () {
        ...
        let existingCookieData = $.cookie('cartItemHashedUserId');

        if (existingCookieData == undefined || existingCookieData == "" || existingCookieData == null) {
            existingCookieData = localStorage.getItem('cartHashUserId');
            $.cookie('cartItemHashedUserId', existingCookieData, { path: '/' });
        }
    }
...
/*function updateCartProducts() {
    let counter = 0;
    if (localStorage.getItem('countCartItem') != null) {
        let cartProducts = JSON.parse(localStorage.getItem('countCartItem'));
        for (let i = 0; i < cartProducts.length; i++) {
            counter = counter + parseInt(cartProducts[i]);
        }
    }
